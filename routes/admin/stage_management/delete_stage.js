...
const { DEFAULT_PROFILE_ADMIN, stage_NAME, stage_VERSION, stage_DESCRIPTION, ROUTE_ADMIN_DELETE_APPLICATION, ROUTE_ADMIN_DELETE_STAGE } = require('../../../config/consts');
...
const SERVICE_TYPE = "admin_delete_stage"
...
router.post('/:stage_id', async function (req, res, next) {
  ...
  if (isInteger(stage_id)) {
    ...
    if (bcontrol.success) {
      let urlcomplete = urlapi + ROUTE_ADMIN_DELETE_STAGE
      let xdata = {
        stage_id: stage_id
      }
