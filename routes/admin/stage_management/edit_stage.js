...
const { DEFAULT_PROFILE_ADMIN, stage_NAME, stage_VERSION, stage_DESCRIPTION, ROUTE_ADMIN_VIEW_APPLICATION, ROUTE_ADMIN_EDIT_APPLICATION, ROUTE_ADMIN_EDIT_STAGE, ROUTE_ADMIN_VIEW_STAGE } = require('../../../config/consts');
...
const SERVICE_TYPE = "admin_edit_stage"

const profile = DEFAULT_PROFILE_ADMIN;
const tabside = getTabSideBase(profile)
const idbloc  = 1
const blocname = tabside[idbloc].texte
const pagename = "Modifier une stage"
...
router.get('/:stage_id', async function (req, res, next) {
  ...
  if (hasError) {
    ...
    res.render(
      profile + "/stage_management/edit_stage", {
      appName: stage_NAME,
...
router.post('/:stage_id', async function (req, res, next) {
  let stage_id = parseInt(req.params.stage_id)
  ...
  if (isInteger(stage_id)) {
    ...
    if (bcontrol.success) {
      let urlcomplete = urlapi + ROUTE_ADMIN_EDIT_STAGE
      let xdata = {
        stage_id: stage_id,
        label: body.label,
        logo: "logolink",
