...
const { DEFAULT_PROFILE_ADMIN, stage_NAME, stage_VERSION, stage_DESCRIPTION, ROUTE_ADMIN_ENABLE_OPERATOR } = require('../../../config/consts');
...
router.get('/:operator_id', async function (req, res, next) {
  ...
  if (isInteger(operator_id)) {
    let urlcomplete = urlapi + ROUTE_ADMIN_ENABLE_OPERATOR
    let xdata = {
