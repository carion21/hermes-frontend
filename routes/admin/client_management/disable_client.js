...
const { DEFAULT_PROFILE_ADMIN, stage_NAME, stage_VERSION, stage_DESCRIPTION, ROUTE_ADMIN_DISABLE_CLIENT } = require('../../../config/consts');
...
const SERVICE_TYPE = "admin_disable_client"
...
router.get('/:client_id', async function (req, res, next) {
  ...
  if (isInteger(client_id)) {
    let urlcomplete = urlapi + ROUTE_ADMIN_DISABLE_CLIENT
    let xdata = {
      client_id: client_id
    }
